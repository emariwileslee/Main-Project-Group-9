...
class nodeClassifier():
    ...
    def addNode(self,node, account_type):#(self,parent_node,username,connection_type):
        print(node)
        ...
        self.exportNode(node, account_type)
        #print(self.node_df)
    ...
    def exportNode(self, node):
        ...
        a = Account(username= node.username, bio=node.bio, avg_likes = node.total_likes, total_followers = node.total_followers, total_following = node.total_following, account_type=account_type)
        a.save()
        #find the parent node 
        #create the connection from the "parent" to the username
        padre = Connection.objects.get(account_type="instagram", username=node.parent_node)
        c = Connection(fro = padre, to = a)
        c.save()
        print("hello this is going swimmingly")
